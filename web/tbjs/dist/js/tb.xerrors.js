...
(function (root, factory) {
  ...
  class MyPromise extends Promise {
    constructor(...params) {
      ...
      this.callStackArr = TB.XErrors.prototype.getStackTraceArray();
    }
  }
  ...
  p.getStackTraceArray = function getStackTraceArray() {
    let callerStackLines = (new Error()).stack;
  ...
  function getLineFileAndColFromStack(stack) {
    ...
    if (stack.indexOf('TypeError: Failed to fetch') > -1) {
      return undefined;
    }
  ...
  p.remoteAuditConnector = function(flushData) {
    ...
    promise
      .then(function(data) {
        ...
        TB.CONFIG.XERRORS_HOOK_ERROR_HANDLER_UI_UPDATE_ERROR(data && data.data && data.data.result);
      })
      .catch(function() {
  ...
  p.errorHandlerUiUpdateError = function errorHandlerUiUpdateError(serverErr) {
    if ( serverErr == undefined) {
      return;
    }

    if (serverErr && serverErr.details && serverErr.details.checksum) {
		if ( ! $('#tb-error-checksum').text() || $('#tb-error-checksum').text() === 'undefined' /* lmao */ ) {
			$('#tb-error-checksum').text(serverErr.details.checksum);
			$('#tb-error-server-system-version').text(serverErr.details.system_version);
			$('#tb-error-audit-code').text(serverErr.details.prefixed_code);
			$('#tb-error-timestamp').text(serverErr.details.timestamp);
			$('#tb-error-diagnostics').text('-');
  ...
  if(TB.CONFIG.HAS_WINDOW) {
    var globalErrorHandling = function(errorEvent) {
      ...
      if (errorEvent.filename && ! errorEvent.filename.match(/\.js$/)) {
        return false;
      }
      ...
      if (errorEvent.error && typeof errorEvent.error === 'object') {
        ...
        if ( ! stack ) {
          if (errorEvent.promise && errorEvent.promise instanceof TB.Promise) {
            stack = errorEvent.promise.callStackArr;
          }
        }
  ...
  TB.CONFIG.XERRORS_HOOK_ERROR_HANDLER_UI_UPDATE_ERROR = TB.CONFIG.XERRORS_HOOK_ERROR_HANDLER_UI_UPDATE_ERROR || globalXerr.errorHandlerUiUpdateError.bind(globalXerr);
