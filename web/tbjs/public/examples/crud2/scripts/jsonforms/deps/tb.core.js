...
TB.CONFIG.XERRORS_LOG_LOCALSTORAGE = (typeof TB.CONFIG.XERRORS_LOG_LOCALSTORAGE !== 'undefined') ? TB.CONFIG.XERRORS_LOG_LOCALSTORAGE  : true;
...
TB.normalizeDate = function( date ) {
  ...
  if ( date instanceof String || typeof date === 'string' ) {
    ...
    date = date.replace( /(^\d{4}(-\d{2})?(-\d{2})?)? (\d{2}(:\d{2})?(:\d{2})?)?/, '$1T$4' )
    date = new Date( date );
  }
